...
let allRains = []
let deafultRaindWidth = 2
let deafultRainHeight = 15
...
let update = () => {
    ...
    while(allRains.length<maximumRainCount && maximumRainInitializationInOneFrame> rainInitCountInThisFrame){
        ...
        let rain = new Rain(deafultRaindWidth * (2-distanceFromCam),
        deafultRainHeight* (2-distanceFromCam),
        (Math.random()/20),
        Math.random()* canvas.width,
        -100, (2-distanceFromCam) *8, "rgba(113,183,219" + ((1-distanceFromCam))+")")
